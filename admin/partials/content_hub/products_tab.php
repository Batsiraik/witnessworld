...
<?php if (isset($_GET['moderated']) && $_GET['moderated'] === '1'): ?>
  <div class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 font-medium">Product updated.</div>
<?php endif; ?>
<?php if (isset($_GET['suspended']) && $_GET['suspended'] === '1'): ?>
  <div class="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900">Product suspended.</div>
<?php endif; ?>
...
<div class="rounded-2xl border border-slate-100 bg-white shadow-panel overflow-hidden">
  <div class="border-b border-slate-100 px-6 py-4 space-y-4">
    <div>
      ...
      <p class="text-sm text-slate-500"><?= htmlspecialchars($tabMeta['desc'], ENT_QUOTES, 'UTF-8') ?> Use <span class="font-semibold text-slate-700">Review &amp; approve</span> for pending items.</p>
    </div>
          ...
          <tr class="bg-white hover:bg-slate-50/80"<?= ww_admin_row_attrs($st) ?>>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex flex-wrap items-center gap-2">
                <button type="button" class="ww-product-modal-open admin-btn admin-btn--<?= htmlspecialchars($btnVariant, ENT_QUOTES, 'UTF-8') ?> admin-btn--sm" data-product-id="<?= $pid ?>"><?= htmlspecialchars($btnLabel, ENT_QUOTES, 'UTF-8') ?></button>
    ...
    <div class="border-t border-slate-100 bg-slate-50/80 px-6 py-4">
      <div id="ww-modal-footer-pending" class="hidden space-y-4">
        ...
        <form id="ww-modal-moderate-form" method="post" action="" class="space-y-3">
          <input type="hidden" name="return_to" value="<?= htmlspecialchars($hubReturn, ENT_QUOTES, 'UTF-8') ?>" />
          <div>
...
(function () {
  ...
  function openModal(id) {
    ...
    document.getElementById('ww-modal-store-link').href = 'store.php?id=' + encodeURIComponent(p.store_id);
