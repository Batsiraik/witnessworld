...
$pendingId = ww_admin_reset_pending_id();
if ($pendingId <= 0) {
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($action === 'resend') {
        try {
            $otp = ww_admin_issue_password_reset_otp($pdo, $pendingId);
            $sent = ww_admin_send_password_reset_otp_email($pdo, $adminRow, $otp);
            $info = $sent
                ? 'A new code was sent to your email. Check spam if it does not arrive soon.'
        ...
        } elseif (ww_admin_verify_password_reset_otp($pdo, $pendingId, $code)) {
            ww_admin_clear_password_reset_otp($pdo, $pendingId);
            unset($_SESSION['admin_reset_pending_id']);
            $_SESSION['admin_reset_allowed_id'] = $pendingId;
            header('Location: reset-password.php');
            exit;
        } else {
...
<body class="min-h-screen bg-slate-100 flex items-center justify-center p-6">
  <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl">
    ...
    <p class="mt-1 text-sm text-slate-500">We sent a 6-digit reset code to <span class="font-semibold text-slate-700"><?= htmlspecialchars($maskedEmail, ENT_QUOTES, 'UTF-8') ?></span>.</p>
    <p class="mt-2 text-xs text-slate-500 leading-relaxed">If you don&apos;t see it, check your <strong class="font-semibold text-slate-600">spam or junk</strong> folder.</p>
    <?php if ($error !== ''): ?>
    ...
    <form method="post" class="mt-6 space-y-4">
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-600">Reset code</label>
        <input
          name="otp"
          inputmode="numeric"
          pattern="[0-9]{6}"
    ...
    <p class="mt-4 text-center">
      <a href="forgot-password.php" class="text-xs font-semibold text-slate-500 hover:text-slate-700">← Start over</a>
      ·
      <a href="login.php" class="text-xs font-semibold text-slate-500 hover:text-slate-700">Sign in</a>
    </p>
  </div>
</body>
</html>
