...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($name !== '' && mb_strlen($name) <= 120) {
            ...
            $slug = trim($slug, '_');
            $maxSt = $pdo->query('SELECT COALESCE(MAX(sort_order),0) FROM service_categories');
            $nextSort = (int) $maxSt->fetchColumn() + 1;
            try {
                $pdo->prepare('INSERT INTO service_categories (name, slug, sort_order) VALUES (?, ?, ?)')
        ...
        if ($catId > 0) {
            $pdo->prepare('UPDATE listings SET category_id = NULL WHERE category_id = ? AND listing_type = \'service\'')->execute([$catId]);
            $pdo->prepare('DELETE FROM service_categories WHERE id = ?')->execute([$catId]);
            $flash = 'deleted';
        }
    } elseif ($action === 'toggle') {
        $catId = (int) ($_POST['cat_id'] ?? 0);
        if ($catId > 0) {
            $pdo->prepare('UPDATE service_categories SET is_active = NOT is_active WHERE id = ?')->execute([$catId]);
            $flash = 'toggled';
        }
        ...
        if ($catId > 0 && $newName !== '' && mb_strlen($newName) <= 120) {
            ...
            $slug = trim($slug, '_');
            try {
                $pdo->prepare('UPDATE service_categories SET name = ?, slug = ? WHERE id = ?')
    ...
    header('Location: service_categories.php' . $qs);
    exit;
}
...
$dbErr = null;
try {
    $rows = $pdo->query('SELECT * FROM service_categories ORDER BY sort_order ASC, id ASC')
        ->fetchAll(PDO::FETCH_ASSOC);
} catch (Throwable) {
    $dbErr = 'Table service_categories does not exist yet. Run the SQL from database/revisions.sql.';
}
...
<?php elseif ($flash === 'deleted'): ?>
  <div class="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900">Category deleted. Service listings under it were un-categorised.</div>
...
<div class="rounded-2xl border border-slate-100 bg-white shadow-panel overflow-hidden">
  <div class="border-b border-slate-100 px-6 py-4 space-y-3">
    <div>
      <h2 class="text-base font-semibold text-slate-900">Service categories</h2>
      <p class="text-sm text-slate-500">Manage the categories users see when posting a professional service listing.</p>
    </div>
    <form method="post" class="flex flex-wrap items-end gap-3">
      <input type="hidden" name="action" value="add" />
      <div>
        <label class="block text-xs font-semibold text-slate-600" for="cat_name">New category</label>
        <input type="text" id="cat_name" name="name" maxlength="120" required class="mt-1 w-60 rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="e.g. Music & Audio" />
      </div>
          ...
          <tr class="bg-white hover:bg-brand-muted/20">
            ...
            <td class="px-6 py-3 text-right">
              <div class="flex flex-wrap items-center justify-end gap-2">
                ...
                <form method="post" class="inline" onsubmit="return confirm('Delete this category? Service listings using it will become uncategorised.')">
